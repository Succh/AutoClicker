...
class HttpServerService : Service() {
        ...
        return try {
            when {
                ...
                path == "/api/run" && method == "POST" -> {
                    ...
                    val config = when {
                        ...
                        !req.json.isNullOrBlank() -> {
                            val c = gson.fromJson(req.json, AutoConfig::class.java)
                ...
                path == "/api/launch" && method == "POST" -> {
                    ...
                    if (service == null) {
                        ...
                        scope.launch {
                            ...
                            Log.i(TAG, "快速启动: ok=$ok, msg=$msg")
                        }
                        ApiResponse(true, "启动指令已发送")
                    }
                }
                ...
                path == "/api/import" && method == "POST" -> {
                    ...
                    if (config != null) ApiResponse(true, config)
                    else ApiResponse(false, null, "JSON 解析失败")
                }
                ...
                path == "/api/port" && method == "POST" -> {
                    ...
                    ApiResponse(true, mapOf("message" to "端口已保存，重启服务后生效", "port" to newPort))
                }
    ...
    data class RunRequest(val configId: String? = null, val name: String? = null, val json: String? = null)
    data class ClickRequest(val text: String? = null, val x: Float? = null, val y: Float? = null)
    data class InputRequest(val text: String? = null)
    data class LaunchRequest(val pkg: String? = null)
    data class PortRequest(val port: Int? = null)
}
